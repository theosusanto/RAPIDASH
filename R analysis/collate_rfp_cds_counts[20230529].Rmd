...
codon_usage_psite_all$wt_RFP_ave = rowMeans(
  codon_usage_psite_all[, c("wt_RFP_1", "wt_RFP_2", "wt_RFP_3")])
codon_usage_psite_all$Nterm_RFP_ave = rowMeans(
  codon_usage_psite_all[, c("Nterm_RFP_1", "Nterm_RFP_2", "Nterm_RFP_3")])
...
codon_usage_asite_all$wt_RFP_ave = rowMeans(
  codon_usage_asite_all[, c("wt_RFP_1", "wt_RFP_2", "wt_RFP_3")])
codon_usage_asite_all$Nterm_RFP_ave = rowMeans(
  codon_usage_asite_all[, c("Nterm_RFP_1", "Nterm_RFP_2", "Nterm_RFP_3")])
...
#plot_df <- codon_usage_asite_all
setDT(plot_df)
plot_df <- plot_df[order(Nterm_RFP_1)][, `:=`(codon, factor(codon, levels = codon))]

correlation <- round(cor(plot_df$Nterm_RFP_1, 
                         plot_df$Nterm_RFP_2), 3)

fit <- lm(plot_df$Nterm_RFP_2 ~ plot_df$Nterm_RFP_1)
outlier_tab <- data.table(predict.lm(fit, interval = "confidence", level = 0.95))[, `:=`(codon, plot_df$codon)][, `:=`(Nterm_RFP_2, plot_df$Nterm_RFP_2)][Nterm_RFP_2 < lwr | Nterm_RFP_2 > upr]
outlier_cod <- as.character(outlier_tab[, `:=`(max_dist, min(abs(Nterm_RFP_2 - lwr), 
                                                             abs(Nterm_RFP_2 - upr))), 
                                        by = 1:nrow(outlier_tab)][order(-max_dist)][1:label_number, codon])
lab_table <- plot_df[, `:=`(codon_out, "")][as.character(codon) %in% outlier_cod, `:=`(codon_out, codon)
                                               ][, `:=`(aa_out, "")][codon_out != "", `:=`(aa_out, aa)]

ggplot(plot_df, aes(x = Nterm_RFP_1, y=Nterm_RFP_2)) +
  geom_smooth(method = "lm", se = TRUE, color = "gray80", fill = "gray80", linetype= 1,
              formula = y ~ x, level = 0.95, fullrange = TRUE) +
  geom_point(alpha = 0.5, size = bs* 0.14) +
  theme_bw(base_size = bs) +
